...
function formatMessage (message) {
  ...
  if (lines[1].indexOf('Module build failed: ') === 0) {
    ...
    const errorText = lines[1].substr('Module build failed: '.length)
    const cleanedLines = []
...
function formatWebpackMessages (json) {
  const errors = json.errors.map(message => `Error in ${formatMessage(message)}`)
  const warnings = json.warnings.map(message => `Warning in ${formatMessage(message)}`)
